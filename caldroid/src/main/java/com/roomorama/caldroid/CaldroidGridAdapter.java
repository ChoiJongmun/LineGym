...
public class CaldroidGridAdapter extends BaseAdapter {
    ...
    protected void setCustomResources(DateTime dateTime, View backgroundView,
        ...
        String year = dateTime.getYear() < 10 ? "0" + dateTime.getYear() : Integer.toString(dateTime.getYear());
        String month = dateTime.getMonth() < 10 ? "0" + dateTime.getMonth() : Integer.toString(dateTime.getMonth());
        String day = dateTime.getDay() < 10 ? "0" + dateTime.getDay() : Integer.toString(dateTime.getDay());
        ...
        Map<String, Integer> textColorForDateTimeMap = (Map<String, Integer>) caldroidData
                .get(CaldroidFragment._TEXT_COLOR_FOR_DATETIME_MAP);
        if (textColorForDateTimeMap != null) {
            // Get textColor for the dateTime
            Integer textColorResource = textColorForDateTimeMap.get(date);
     ...
     * @param holder
     */
    protected void customizeTextView(int position, ViewHolder holder) {
        // Get the padding of cell so that it can be restored later
        int topPadding = holder.mDayTv.getPaddingTop();
        int leftPadding = holder.mDayTv.getPaddingLeft();
        int bottomPadding = holder.mDayTv.getPaddingBottom();
        int rightPadding = holder.mDayTv.getPaddingRight();

        // Get dateTime of this cell
        DateTime dateTime = this.datetimeList.get(position);

        holder.mDayTv.resetCustomStates();
        resetCustomResources(holder.mDayTv);
        ...
        if (dateTime.getMonth() != month) {
            holder.mDayTv.addCustomState(CellView.STATE_PREV_NEXT_MONTH);
        }
        ...
        if ((minDateTime != null && dateTime.lt(minDateTime))
            ...
            holder.mDayTv.addCustomState(CellView.STATE_DISABLED);
        }

        // Customize for selected dates
        if (selectedDates != null && selectedDatesMap.containsKey(dateTime)) {
            holder.mDayTv.addCustomState(CellView.STATE_SELECTED);
        }

        holder.mDayTv.refreshDrawableState();

        // Set text
        holder.mDayTv.setText(String.valueOf(dateTime.getDay()));

        // Set custom color if required
        setCustomResources(dateTime, holder.mDayTv, holder.mDayTv);
...
//		CellView cellView;
        ...
        if(convertView == null) {
            ...
            convertView = inflater.inflate(R.layout.item_calendar_day, null);
            viewHolder = new ViewHolder();
            viewHolder.mDayTv = (CellView) convertView.findViewById(R.id.calendar_tv);
            convertView.setTag(viewHolder);
        }else {
            viewHolder = (ViewHolder)convertView.getTag();
        }
//		if (convertView == null) {
//			final int squareDateCellResource = squareTextViewCell ? R.layout.item_calendar_day : R.layout.normal_date_cell;
//			cellView = (CellView) localInflater.inflate(squareDateCellResource, parent, false);
//		} else {
//			cellView = (CellView) convertView;
//		}

		customizeTextView(position, viewHolder);
