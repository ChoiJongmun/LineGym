    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        if (mChart.isDrawBarShadowEnabled()) {
            ...
            for (int i = 0, count = Math.min((int)(Math.ceil((float)(dataSet.getEntryCount()) * phaseX)), dataSet.getEntryCount());
                ...
                c.drawRoundRect(mBarShadowRectBuffer, 1,1,mShadowPaint);
            }
        }
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            RectF tmp = new RectF();
            tmp.set(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2], buffer.buffer[j + 3]);
            c.drawRoundRect(tmp,1,1, mRenderPaint);

            if (drawBorder) {
                c.drawRoundRect(tmp,1,1, mBarBorderPaint);
...
//                        buffer.buffer[j + 3], mBarBorderPaint);
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
            ...
            c.drawRoundRect(mBarRect,1f,1f,mHighlightPaint);
        }
    }
